...
class ReportSalesByCustomer {
	...
	public function run($args, Closure $getSelectFields) {
		...
		$query = $item->selectRaw('IFNULL(customers.name, "Walk-in" ) as item_name, sum(IF(products.type LIKE  "%product%", total_amount, 0)) as product_amount, sum(IF(products.type LIKE  "%service%", total_amount, 0)) as service_amount, sum(total_amount) as total_amount')
