...
class ReportCommissionDetailed {
	...
	public function run($args, Closure $getSelectFields) {
		...
		$query = Item::join($documents, $documents . '.id', '=', $items . '.trxn_id')
			...
			->where($items . '.type', 'commission')
			->where($where)
			->selectRaw('DATE(' . $documents . '.date) as item_date, users.name as user_name, products.name as item_name, sum(total_amount) as total_amount')
