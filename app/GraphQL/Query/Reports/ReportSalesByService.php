...
class ReportSalesByService {
	...
	public function run($args, Closure $getSelectFields) {
		...
		$item = Item::join($documents, $documents . '.id', '=', $items . '.trxn_id')
			...
			->where('products.type', 'LIKE', '%service%')
			->where($where)
			->selectRaw('products.name as item_name, products.type as product_type, category.name as category_name, sum(ABS(qty)) as qty, sum(total_amount) as total_amount');
