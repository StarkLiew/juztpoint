...
class ReportSalesByMonth {
	...
	public function run($args, Closure $getSelectFields) {
		...
		$query = $item->selectRaw('CONCAT(MONTHNAME(documents.date),", ", YEAR(documents.date)) as item_date, sum(IF(products.type LIKE  "%product%", total_amount, 0)) as product_amount, sum(IF(products.type LIKE  "%service%", total_amount, 0)) as service_amount, sum(total_amount) as total_amount')
			->groupBy(DB::raw('CONCAT(MONTHNAME(documents.date),", ", YEAR(documents.date))'));
