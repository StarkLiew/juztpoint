...
class ReportSalesByStore {
	...
	public function run($args, Closure $getSelectFields) {
		...
		$item = Item::join($documents . ' as documents', 'documents.id', '=', $items . '.trxn_id')
			...
			->join($settings . ' as store', 'store.id', '=', 'documents.store_id')
			->where($items . '.type', 'item')
			->where($where);

		$sum = $item->sum('total_amount');
		$count = $item->sum('trxn_id');

		$query = $item
			->selectRaw('store.name as item_name, sum(IF(products.type LIKE  "%product%", total_amount, 0)) as product_amount, sum(IF(products.type LIKE  "%service%", total_amount, 0)) as service_amount, sum(total_amount) as total_amount')
			->groupBy('store.name');
