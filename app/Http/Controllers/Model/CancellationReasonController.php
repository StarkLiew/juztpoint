...
class CancellationReasonController extends Controller
{
   ...
   public function __construct()
    ...
    public function create(Request $request){
        ...
        $data = new CancellationReason($request->all());
    ...
    public function all(Request $request){
      ...
      $model = \App\Models\CancellationReason::orderBy($option['by'], $option['order']);
   ...
   public function detail($id, Request $request){
      if(!empty($id)){
          $result = CancellationReason::where("id", "=", $id)->first();
  ...
  public function destroy(Request $request){
       ...
       if ($request->has('password')) {
          if (UC::isCurrentUser($request->input('password'))) {
             $affectedRows = CancellationReason::onlyTrashed()->forceDelete();
             if($affectedRows){
