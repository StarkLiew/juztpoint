...
class CategoryController extends Controller
{
   ...
   public function __construct()
    ...
    public function all(Request $request){
      ...
      $model = Category::where('service', $service)->orderBy($option['by'], $option['order']);
  ...
  public function destroy(Request $request){
       ...
       if ($request->has('password')) {
          if (UC::isCurrentUser($request->input('password'))) {
             $affectedRows = Category::onlyTrashed()->forceDelete();
             if($affectedRows){
   ...
   private function isService(){
       ...
       if(strpos($uri, 'service_categories')) return true;
       return false;
    }       
