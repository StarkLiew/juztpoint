...
class UserController extends Controller
{
    ...
    public function register(Request $request){
        ...
        if($request->has('pin')) $user->pin = $request->input('pin');
       ...
       /* try{
             ...
             $user->save();
             return $this->callback($request, $user);
        }catch(\Exception $e){
          if($e->getCode() === '23000'){
    ...
    public function create(Request $request){
        ...
        if($data->save()){
           if(!empty($form->role_id)) $data->roles()->attach($form->role_id);
            $data->load('roles');
    ...
    public function update($id, Request $request){
        ...
        if($data->update($form)){
           ...
           if(!empty($form['role_id'])) $data->attachRole($form['role_id']);
           $data->load('roles');
           return response()->json(['msg'=>'Saved', 'data'=>$data], 200);
        } else {
           return response()->json(['msg'=>'Fail', 'data'=>[]], 400);
    ...
    private function validateInput($request){
         ...
         if($request->has('first_name')){
            $name = trim($request->input('first_name'));
            if(strlen($name) < 3){
               $this->errors['first_name'] = ["msg"=>"First name length too short"]; 
            }

         }else{
               $this->errors['first_name'] = ["msg"=>"First name is required"]; 
         }
         ...
         if($request->has('password')){
            ...
            if($this->append) $this->errors['password'] = ["msg"=>"Password cannot be empty"]; 
         }
         if($request->has('pin')){
              $pin = trim($request->input('pin'));
              if(strlen($pin) !== 4)  return "Pin length too short";
