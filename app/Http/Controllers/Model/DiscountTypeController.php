...
class DiscountTypeController extends Controller
{
  ...
  public function __construct()
    ...
    public function create(Request $request){
        ...
        $data = new DiscountType($request->all());
    ...
    public function all(Request $request){
      ...
      $model = DiscountType::orderBy($option['by'], $option['order']);
   ...
   public function detail($id, Request $request){
      if(!empty($id)){
          $result = DiscountType::where("id", "=", $id)->first();
  ...
  public function destroy(Request $request){
       ...
       if ($request->has('password')) {
          if (UC::isCurrentUser($request->input('password'))) {
             $affectedRows = Clock::onlyTrashed()->forceDelete();
             if($affectedRows){
