...
class AccountController extends Controller
{
    ...
    public function create(Request $request){
        ...
        $data = new Account($request->all());
        $data->user_id = Auth::user()->id;
        $data->supplier = $this->isSupplier();
    ...
    public function all(Request $request){
      ...
      $model = \App\Models\Account::where('supplier',$supplier)->orderBy($option['by'], $option['order']);
  ...
  public function destroy(Request $request){
       ...
       if ($request->has('password')) {
          if (UC::isCurrentUser($request->input('password'))) {
             $affectedRows = Account::onlyTrashed()->forceDelete();
             if($affectedRows){
    ...
    private function isSupplier(){
       ...
       if(strpos($uri, 'suppliers')) return true;
       return false;
    }
