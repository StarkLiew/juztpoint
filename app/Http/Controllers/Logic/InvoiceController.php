...
class InvoiceController extends Controller {
	...
	public function all(Request $request) {
		...
		if ($filtername !== '') {
			$model = $model->where('id', 'like', '%' . $filtername . '%');
		}
...
//         DB::enableQueryLog();
	...
	public function calcCommission($invoice, $details = [], $lineId = '') {
		...
		foreach ($details as $detail) {
			...
			if ($commRate->fix) {
				...
				if ($detail->assistant_id) {
					...
					if (empty($lineId)) {
						$commission2->inv_detail_id = $detail->id;
					} else {
						$commission2->inv_detail_id = $lineId;
					}

					//$commission2->inv_detail_id = $lineId || $detail->id;
					if ($lineId) {
						$commission2->appointment_detail_id = $detail->id;
					}
		...
		if (!empty($id)) {
			return response()->json($this->commissions);
		}
