...
export const actions = {
    async fetchServices({ commit }) {
        try {
            const { data } = await axios.get(graphql.path('query'), { params: { query: '{products(type: "%service%", limit:0, page: 1){ data{id, type, thumbnail, name, sku, category{id, name}, commission{id, name,  properties{rate, type}},allow_assistant, tax{id, name, properties{rate, code}}, variants{name, value}, composites{id, name},properties{price, thumbnail, contain, color, duration, stocks{name, price}}}}}' } })
