<template>
    <crud title="Stores" :headers="headers" :items='items' sort-by="name" :refresh="retrieve" :default-item="defaultItem" :options.sync="options" :save-method="save" :remove-method="remove" :server-items-length="count" :loading="loading" loading-text="Loading..." :export-fields="exportFields">
...
export default {
    ...
    methods: {
        async retrieve(search, options, noCommit = false) {
            ...
            const results = await this.$store.dispatch('setting/fetch', { type: 'store', search, limit: itemsPerPage, page, sort: sortBy, desc: sortDesc, noCommit })
