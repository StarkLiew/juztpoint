    ...
    methods: {
        async editDialogHandler(val) {
            ...
            if (!!val) {
                ...
                this.standards = await this.$store.dispatch('product/fetch', { type: 'product%', search: '', limit: 0, page: 1, sort: true, desc: false, noCommit: true, customFields: 'id, name, variants{name, value}' })
            }
