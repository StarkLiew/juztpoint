...
export const actions = {
  async fetchProducts({ commit }) {
    try {
      const { data } = await axios.get(graphql.path('query'), {params: { query: '{products(type: "product"){ id,   name,  category{id, name}, commission{id, name, properties{rate, type}},tax{id, name, properties{rate, code}}, properties{qty, price, thumbnail, color}}}'}})
